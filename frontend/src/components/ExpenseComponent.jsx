...
function Exp() {
  ...
  const handleEdit = (expense) => {
    setForm({
      ...
      expense: expense.expense,
      expenseType: expense.expenseType,
      expDate: new Date(expense.expDate).toISOString().split("T")[0],
      Amount: expense.Amount,
    });
    setEditing(true);
...
function Exp() {
  ...
  const handleIdKeyDown = (e) => {
    ...
    // During typing: first 3 letters must be 'Exp'
    if (value.length < 3) {
      const expLetters = ["E", "x", "p"];
      if (e.key !== expLetters[value.length]) {
        e.preventDefault();
      }
    } else {
      // After "Exp", only digits are allowed
      if (!/[0-9]/.test(e.key)) {
        e.preventDefault();
      }
    }
