...
const SaleForm = () => {
  ...
  const fetchCustomer = async () => {
    try {
      const response = await axios.get(`http://localhost:8070/customer/${customerId}`);
  ...
  const fetchSale = async () => {
    try {
      const response = await axios.get(`http://localhost:8070/sale/${id}`);
      setFormData(response.data);
      if (response.data.customer) {
        const customerResponse = await axios.get(`http://localhost:8070/customer/${response.data.customer}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`http://localhost:8070/sale/update/${id}`, saleData);
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="col-md-6">
            ...
            {!customerId && !id && (
              <small className="text-muted">
                Note: This form should be accessed from a customer page to associate the sale.
              </small>
            )}
          </div>
        ...
        <div className="d-flex justify-content-between">
          ...
          <button
            ...
            disabled={loading || formData.products.length === 0}
          >
            {loading ? "Saving..." : id ? "Update Sale" : "Record Sale"}
          </button>
        </div>
      </form>
    </div>
